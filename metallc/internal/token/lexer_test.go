...
func TestLexer(t *testing.T) {
	...
	tests := []struct {
		...
		{"amp immediate (no trailing space)", "&", []want{{AmpImmediate, "", "1:1"}}},
		{"amp immediate ref", "&x", []want{
			{AmpImmediate, "", "1:1"},
			{Ident, "x", "1:2"},
		}},
		{"amp immediate mut ref", "&mut x", []want{
			{AmpImmediate, "", "1:1"},
			{Mut, "", "1:2-1:4"},
			{Whitespace, " ", "1:5"},
			{Ident, "x", "1:6"},
		}},
		{"amp spaced mut is binary, not a ref", "& mut", []want{
			{Amp, "", "1:1"},
			{Whitespace, " ", "1:2"},
			{Mut, "", "1:3-1:5"},
		}},
		...
		{"minus eq vs negative number", "x -= -5", []want{
			{Ident, "x", "1:1"},
			{Whitespace, " ", "1:2"},
			{MinusEq, "", "1:3-1:4"},
			{Whitespace, " ", "1:5"},
		...
		{"number (negative hex)", `-0xff`, []want{{Number, "-0xff", "1:1-1:5"}}},
		{"number (octal)", `0o755`, []want{{Number, "0o755", "1:1-1:5"}}},
		{"number (negative octal)", `-0o17`, []want{{Number, "-0o17", "1:1-1:5"}}},
		{"number (binary)", `0b1010_1010`, []want{{Number, "0b1010_1010", "1:1-1:11"}}},
		{"number (negative binary)", `-0b101`, []want{{Number, "-0b101", "1:1-1:6"}}},
		...
		{"string escape newline", `"line1\nline2"`, []want{{String, "line1\nline2", "1:1-1:14"}}},
		{"string escape tab", `"\thello\t"`, []want{{String, "\thello\t", "1:1-1:11"}}},
		{"string escape null", `"\0"`, []want{{String, "\x00", "1:1-1:4"}}},
		{"string escape carriage return", `"\r"`, []want{{String, "\r", "1:1-1:4"}}},
		{"string escape backslash", `"path\\to\\file"`, []want{{String, "path\\to\\file", "1:1-1:16"}}},
		{"string escape double quote", `"\""`, []want{{String, `"`, "1:1-1:4"}}},
		{"string escape hex", `"\xFF"`, []want{{String, "\u00FF", "1:1-1:6"}}},
		{"string escape hex lowercase", `"\x0a"`, []want{{String, "\n", "1:1-1:6"}}},
		{"string escape hex uppercase", `"\x0A"`, []want{{String, "\n", "1:1-1:6"}}},
		{"string escape hex null", `"\x00"`, []want{{String, string(rune(0)), "1:1-1:6"}}},
		{"string escape unicode", `"\u{20AC}"`, []want{{String, "\u20AC", "1:1-1:10"}}},
		{"string escape unicode single digit", `"\u{0}"`, []want{{String, string(rune(0)), "1:1-1:7"}}},
		{"string escape unicode supplementary", `"\u{1F600}"`, []want{{String, "\U0001F600", "1:1-1:11"}}},
		{"string escape unicode lowercase", `"\u{ff}"`, []want{{String, "\u00FF", "1:1-1:8"}}},
		{"string with single quotes", `"'hello'"`, []want{{String, "'hello'", "1:1-1:9"}}},
		{"string unknown escape", `"hello\aworld"`, []want{{Error, `unknown escape sequence '\a'`, "1:1-1:14"}}},
		{"string invalid hex escape", `"\xGG"`, []want{{Error, "invalid byte escape sequence", "1:1-1:6"}}},
		{"string invalid unicode escape", `"\u{ZZZZ}"`, []want{{Error, "invalid unicode escape sequence", "1:1-1:10"}}},
		{
			"string invalid unicode escape empty",
			`"\u{}"`,
			[]want{{Error, "invalid unicode escape sequence", "1:1-1:6"}},
		},
		{
			"string invalid unicode escape unclosed",
			`"\u{00FF"`,
			[]want{{Error, "invalid unicode escape sequence", "1:1-1:9"}},
		},
